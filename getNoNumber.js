...
async function main6() {
	...
	const a = JSON.parse(
		fs.readFileSync('./database/updateQN.json').toString()
	);
	let data = [...new Set(a)];
	const file = {
		errorList: [],
...
async function main2() {
	let data = JSON.parse(
		fs.readFileSync('./database/updateInfo.json').toString()
	);
...
async function main3() {
	let data = JSON.parse(
		fs.readFileSync('./database/updateInfo2.json').toString()
	);
	console.log(data.length);
	let err = [];
	for (let i = 0; i < data.length; i++) {
...
async function main4() {
	const pic = fs
		...
		.map((el) => el.split('.')[0].padStart(8, '0'));
	let data = JSON.parse(
		fs.readFileSync('./database/updateError1.json').toString()
	);
	for (let i = 0; i < data.length; i++) {
		const target = data[i];
...
async function getCardsByInput() {
	...
	const makeObj = async (num, info) => {
		let obj = {};
		const res = await useReptileByForm(
			'http://220.134.173.17/gameking/card/ocg_list.asp',
			{
				form_data1: num,
			}
...
function makeStatus() {
	...
	fs.writeFileSync('./database/productInformation.json', JSON.stringify(db));
}
