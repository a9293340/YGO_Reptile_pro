...
async function main() {
	...
	await useReptileByForm('http://220.134.173.17/gameking/card/ocg_list.asp', {
		...
		for (let p = 11; p < page; p++) {
			...
			for (let j = 9; j < namesInfo.length; j++) {
				// if (j) break;
				...
				if (
					...
					const tart = $s(
						'tr[bgcolor="#E8F4FF"] td[width="05%"][bgcolor="#ffff99"]'
					);
...
const goNext = async (src) => {
	const effect_url = await useReptileTargetUrl(
		`http://220.134.173.17/gameking/card/${src}`
	);
	...
	try {
		...
		for (let p = 0; p < page; p++) {
			...
			card_id_arr = [
				...card_id_arr,
				...useReptile2Split(
					$s('tr[bgcolor="#E8F4FF"] td[width="09%"][align="center"]')
