...
export const reptileCardInfo = async (file) => {
	...
	const getDesc = async (rarity, $) => {
		const effect_url = await useReptileTargetUrl(
			`http://220.134.173.17/gameking/card/${$(rarity)
				.children()
				.attr('href')}`
		);
	...
	const makeMoreData = (
		...
		for (let i = 0; i < card_id_arr.length; i++) {
			...
			const card_product_information_type =
				card_product_information_type_arr[i];
			const coolName = names[i].indexOf('異圖') !== -1;
			const card_rarity = `${coolName ? '異圖-' : ''}${RarityTransfer(
				card_rarity_arr[i]
			)}`;
			if (
				arr.find(
					(el) =>
						el.product_information_type ===
							card_product_information_type &&
				...
				if (
					arr[set].rarity.findIndex((el) => el === card_rarity) === -1
				)
	...
	const errorControl = (spinner, cardInfo, set, file) =>
		spinner
			.error({
				...
				)} Current progress [${set + 1}/${
					file.lastAdd.length
				}] ${chalk.blue(
					` ${
						parseInt(((set + 1) / file.lastAdd.length) * 1000000) /
						10000
					}% `
				)}`,
			})
	...
	for (let set = 0; set < file.lastAdd.length; set++) {
		...
		try {
			await useReptileByForm(
				...
				//! 種族 & 分類
				const card_race = $(
					'tr[bgcolor="#E8F4FF"] td[width="10%"][align="center"]'
				);
				...
				//! 攻擊 & 防禦
				const card_atk = $(
					'tr[bgcolor="#E8F4FF"] td[width="07%"][align="center"]'
				);
				...
				//! page
				const page = $('tr[bgcolor="#C1E0FF"] > td > select')
					.first()
					.children().length;
				for (let p = 0; p < page; p++) {
					...
					card_id_arr = [
						...
						).map((el) => checkIdRules(el)),
					];
					//! 稀有度 (多)& 屬性
					const rarity_attribute = $s(
						'tr[bgcolor="#E8F4FF"] td[width="05%"][align="center"]'
					);
					for (let i = 0; i < rarity_attribute.length; i++) {
						...
						if (jud_correct_info($s(rarity).text()))
							jud_correct_info($s(rarity).text()) === 1
								? (cardInfo.attribute = useReptile2Str(
										$s(rarity).text()
								  ))
								: card_rarity_arr.push(
										useReptile2Str($s(rarity).text())
								  );
						else if (
							$s(rarity).text() === '內容' &&
							cardInfo.effect === ''
				...
				//! 產品包代號(多)
				card_product_information_type_arr = card_id_arr.map(
					(el) => el.split('-')[0]
				);
				...
				for (let i = 0; i < finalData.length; i++) {
					const cards = await MongooseCRUD('R', 'cards', {
						id: finalData[i].id,
					});
					if (!cards.length)
					...
					const image = {
						...
						photo: `data:image/jpeg;base64,${await img2base(
							`./pics/${number}.jpg`
						)}`,
					};
				...
				)} Success! Current progress [${set + 1}/${
					file.lastAdd.length
				}] ${chalk.blue(
					` ${
						parseInt(((set + 1) / file.lastAdd.length) * 1000000) /
	...
	//! 韓文 out!
	final = final.filter(
		(el) => el.id.indexOf('-KR') === -1 && el.id.indexOf('-EN') === -1
	);
...
export const reptileOptions = async () => {
	...
	let final = {};
	let errorMsg = [];
	const res = await useReptileTargetUrl(
		'http://220.134.173.17/gameking/card/ocg_index.asp'
	);
	...
	const productInformation = await makeProductInfoPackType(
		final.product_information_type
	);
