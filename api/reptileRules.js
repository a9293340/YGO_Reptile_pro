...
const getRulesAndInfo = async (file, jud) => {
	...
	// 看有幾頁page
	let page = 1;
	if ($(".page_num ").children().length <= 5)
		page = $(".page_num ").children().length;
	else {
		page = parseInt(
...
const hasJp = async () => {
	...
	for (let i = 0; i < allData.length; i++) {
		...
		try {
			fs.writeFileSync(
				allData[i],
				JSON.stringify((await getRulesAndInfo(file, 0), null, 2)).file
			);
			console.log(
				`${allData[i]} final! ${i}/${allData.length}(${Math.floor(
					(new Date() - now) / 1000
				)}s)`
			);
		} catch (error) {
...
export const noJp = async () => {
	if (!fs.readdirSync("./newRule").find((el) => el.indexOf("rule.json") !== -1))
		return;
	...
	for (let i = 0; i < cards.length; i++) {
		...
		try {
			...
			console.log(
				"New ",
				card.number,
				errorCount === card.ids.length ? " Failed" : "Ok"
			);
		} catch (error) {
...
export const hasJPUpdate = async () => {
	...
	console.log(updates);
	fs.writeFileSync(
		`./newRule/change_${new Date().toDateString()}.json`,
