...
const hasJp = async () => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    try {
      fs.writeFileSync(allData[i], JSON.stringify(await getRulesAndInfo(file, 0), null, 2));
      console.log(
...
const noJp = async numbers => {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    // 改成寫到資料庫
    fs.writeFileSync(`./rules/__${card.number}.json`, JSON.stringify(info, null, 2));
  }
};
...
const hasJPUpdate = async () => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    const file = JSON.parse(fs.readFileSync(allData[i]).toString());
    fs.writeFileSync(allData[i], JSON.stringify(await getRulesAndInfo(file, 1), null, 2));
  }
  console.log('over');
};
