				...
				if (targets.length) {
					...
					if (rarity.length > 1)
						...
						prices = prices.filter((el) =>
							checkWordsFunction(el.ProdName, rar)
						);
					// console.log(prices);
					prices = prices
					...
					if (
						!Number.isInteger(price.price_avg) &&
						price.price_avg === Infinity
					)
						errorControls("avg");
					if (
						!Number.isInteger(price.price_lowest) &&
						price.price_lowest === Infinity
					)
						errorControls("lowest");
				} else {
					errorControls("avg");
