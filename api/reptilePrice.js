...
const rarityWordsFunction = (rar, rarity) => {
  ...
  else if (['金亮KC紋', 'KC紋'].find(el => el === rar)) return '+KC';
  ...
  else if (rar.indexOf('古紋鑽') !== -1) return '+古紋鑽';
...
const checkWordsFunction = (tar, rar) => {
  ...
  if (rar.indexOf('異圖') !== -1) {
    ...
    if (['方鑽', '點鑽'].find(el => el === rar)) return tar.indexOf('普卡') !== -1;
    else if (['凸版浮雕', '浮雕'].find(el => el === rar)) return tar.indexOf('浮雕') !== -1;
    else if (['20th紅鑽', '紅鑽'].find(el => el === rar)) return tar.indexOf('紅鑽') !== -1;
    else if (['25th金鑽', '金鑽'].find(el => el === rar)) return tar.indexOf('金鑽') !== -1;
    else if (['金亮KC紋'].find(el => el === rar)) return tar.indexOf('KC') !== -1;
    else if (['普卡放射鑽'].find(el => el === rar)) return tar;
  ...
  for (let c = 0; c < cardInfo.length; c++) {
    ...
    try {
      for (let r = 0; r < rarity.length; r++) {
        ...
        price.rarity = rar;
        const rarityWords = rarityWordsFunction(rar, rarity);
        // console.log(rarityWords);
