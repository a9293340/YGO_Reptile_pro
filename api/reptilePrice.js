...
export const reptilePrice = async () => {
	...
	// 	const spinner = createSpinner().start({
	// 		text: `Upload Card ID : ${chalk.whiteBright.bgMagenta(
	// 			cardInfo[c].id
	// 		)}  `,
	// 	});
	// 	const totalSpendTime = `Total Spend ${chalk.bgGray(
	...
	// 		spinner
	// 			.success({
	// 				text: `Upload Card ${chalk.whiteBright.bgGreen(
	// 					` ${cardInfo[c].id}`
	// 				)} Success! (${totalSpendTime} sec)`,
	// 			})
	// 			.clear();
	// 	} catch (error) {
	// 		spinner
	// 			.error({
	// 				text: `Card ID : ${chalk.white.bgCyanBright(
	// 					`${cardInfo[c].id} upload Failed!`
	// 				)}(${totalSpendTime} sec)`,
	// 			})
	// 			.clear();
	// 	}
...
const weight_function = (prices, type) => {
	...
	let avg = jud
		? prices.reduce((a, b) =>
				b < q1 ? a + b * 1.25 : b > q3 ? a + b * 0.75 : a + b
		  ) / prices.length
		: prices.reduce((a, b) => (b > q3 ? a + q3 : a + b)) / prices.length;
	let lowest = prices[0];
	let low = jud
		? prices.reduce((a, b) =>
				b < q1 ? a + b * 1.25 : b < (q1 + q3) / 2 ? a + b : a + b * 0.75
		  ) / prices.length
